...
@php $que = DB::table('fmt_mcqp_ques')->where('id', $message)->first(); @endphp
<div class="fixed z-10 inset-0 overflow-y-auto hidden" id="modalMCQP{{$que->id}}">
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      -->
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full relative -mx-8" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            <a onclick="closemodalMCQP({{$message}})" class="p-2 w-8 h-8 bg-gray-600 text-white rounded-full absolute right-0 -top-10 -mr-2 -mt-2 z-40" href="javascript:void(0);">x</a>
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <form action="{{route('fmt.mcqp.update', $que->id)}}" method="post" enctype="multipart/form-data">
                    @if ($errors ?? '')
                        <div class="my-4">
                    ...
                    @endif
                    @csrf
                    <div class="text-xl">Edit MCQp Question {{$message}}</div>
                    ...
                    <div class="flex flex-wrap -mx-4 my-2">{{-- flex-wrap --}}
                        <div class="w-full px-4">{{-- w-1/3 --}}
                            <div class="my-2">
                                <label class="bloc" for="">Question</label>
                                <input type="file" name="question_media" class="w-full border border-gray-500 rounded-lg p-2">
                            </div>
                        </div>{{-- //w-1/3 --}}
                    </div>{{-- //flex-wrap --}}
                    <div class="my-2">
                   ...
                   {{--  <div class="my-2">
                    ...
                    @php $answers = DB::table('fmt_mcqp_ans')->where('question_id', $que->id)->pluck('id'); @endphp
                    @foreach ($answers as $ans)
                        @php $answer = DB::table('fmt_mcqp_ans')->where('id', $ans)->first(); @endphp
                        {{--  --}}
                        <div class="flex flex-wrap px-2 my-4 border border-gray-500" id="ans1">
                            <div class="w-1/2">
